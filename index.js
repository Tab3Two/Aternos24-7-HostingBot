...
const { SocksClient } = require('socks'); // <-- THÊM THƯ VIỆN SOCKS ĐỂ LÁCH BLOCK IP

// ============================================================
// PROXY CONFIGURATION (Cấu hình Proxy SOCKS5 ở đây hoặc qua Environment Variables)
// ============================================================
const PROXY_HOST = process.env.PROXY_HOST || "YOUR_PROXY_IP"; // Thay bằng IP Proxy của bạn
const PROXY_PORT = parseInt(process.env.PROXY_PORT || "1080"); // Port Proxy
const PROXY_USER = process.env.PROXY_USER || ""; // Tài khoản proxy (nếu có)
const PROXY_PASS = process.env.PROXY_PASS || ""; // Mật khẩu proxy (nếu có)

// Tự động bật proxy nếu bạn đã điền IP proxy (không còn là "YOUR_PROXY_IP" hay chuỗi rỗng)
const USE_PROXY = process.env.USE_PROXY === 'true' || (PROXY_HOST && PROXY_HOST !== "YOUR_PROXY_IP" && PROXY_HOST !== "");
...
// Hàm khởi tạo Bot chính (đã tích hợp logic Proxy và Direct connection)
function createBot() {
  if (isReconnecting) {
  ...
  // Cleanup bot cũ để tránh lỗi tạo bóng ma bot (ghost bot)
  if (bot) {
    clearAllIntervals();
    try {
  ...
  console.log(`[Bot] Preparing bot instance...`);

  // Nếu cấu hình Proxy hợp lệ, khởi tạo luồng socket đi qua SOCKS5 Proxy trước
  if (USE_PROXY) {
    console.log(`[Bot] Connecting via SOCKS5 Proxy: ${PROXY_HOST}:${PROXY_PORT}`);
    ...
    if (PROXY_USER && PROXY_PASS) {
      proxyOptions.proxy.userId = PROXY_USER;
      proxyOptions.proxy.password = PROXY_PASS;
    }

    SocksClient.createConnection(proxyOptions, (err, info) => {
      if (err) {
        console.log(`[Bot] Proxy connection failed: ${err.message}`);
        // Nếu proxy tèo, lên lịch tự động reconnect
        scheduleReconnect();
        return;
      }

      console.log(`[Bot] SOCKS5 connection established. Initializing Mineflayer...`);
      initMineflayer(info.socket);
    });
  } else {
    console.log(`[Bot] Connecting directly to ${config.server.ip}:${config.server.port} (Proxy disabled hoặc chưa cài đặt)`);
    initMineflayer(null);
  }
}

// Tách hàm khởi tạo Mineflayer để dễ dàng nhận luồng Stream Socket từ proxy
function initMineflayer(socketStream) {
  try {
    ...
    // Nếu kết nối qua Proxy, nạp stream của Proxy Socket vào cho bot chạy đè lên
    if (socketStream) {
      botOptions.stream = socketStream;
    }
